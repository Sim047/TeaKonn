...
export default function Discover({ token, onViewProfile }: any) {
  ...
  const submitJoinRequest = async (eventId: string, txCode: string) => {
    try {
      ...
      const response = await axios.post(
        `${API}/api/bookings-simple/create`,
  ...
  const shouldShowSportsGrid =
    showAllSports || (selectedCategory && selectedCategory !== 'All') || searchQuery.trim();
      ...
      <div className="max-w-7xl mx-auto mb-12">
        <div className="flex items-center justify-between mb-6">
          ...
          >
            ...
            <ArrowRight
              className={`w-4 h-4 transition-transform ${showAllSports ? 'rotate-90' : ''}`}
            />
          </button>
        </div>
        ...
        {shouldShowSportsGrid && (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {displaySports.map((sport) => (
              <div key={sport.name} className="cursor-pointer group relative">
                <div
                ...
                >
                  ...
                  {event.organizer && (
                    ...
                    >
                      ...
                      <span>
                        Organized by{' '}
                        <span className="font-semibold">{event.organizer.username}</span>
                      </span>
                    </div>
                  )}
