...
const SAMPLE_AVATAR = 'https://ui-avatars.com/api/?name=User&background=0D8ABC&color=fff';
...
export default function App() {
  ...
  const [showLongPressHintChat, setShowLongPressHintChat] = useState<boolean>(() => {
    try {
      return !localStorage.getItem('auralink-hint-chat-longpress');
    } catch {
      return true;
    }
  });
  ...
  const [profileFollowersCount, setProfileFollowersCount] = useState<number | null>(null);
  const [profileFollowingCount, setProfileFollowingCount] = useState<number | null>(null);
  ...
  async function saveEdit(id: string) {
    ...
    setMessages((m) =>
      m.map((msg) => (msg._id === id ? { ...msg, text: editingText, edited: true } : msg)),
    );
    setEditingMessageId(null);
  ...
  useEffect(() => {
    ...
    socket.on('receive_message', async (msg: any) => {
      setMessages((m) => {
        ...
        const filtered = m.filter((existing) => {
          ...
          if (
            existing.sender?._id === msg.sender?._id &&
            existing.text === msg.text &&
            Math.abs(new Date(existing.createdAt).getTime() - new Date(msg.createdAt).getTime()) <
              5000
          ) {
            return false;
          }
    ...
    socket.on(
      ...
      ({ bookingId, status, approvalStatus, message, paymentVerified }: any) => {
        ...
        if (user && socket) {
          list.forEach((msg: any) => {
            if (
              ...
              !msg.readBy?.some((id: any) => String(id) === String(user._id))
            ) {
              socket.emit('message_read', { messageId: msg._id, userId: user._id });
            }
          });
  ...
  async function deleteMessageForEveryone(id: string) {
    try {
      await axios.delete(`${API}/api/messages/${id}/force`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setMessages((msgs) => msgs.filter((x) => x._id !== id));
    } catch (e) {
  ...
  async function showProfile(userOrId: any) {
    ...
    const id = typeof userOrId === 'string' ? userOrId : userOrId?._id || userOrId?.id;
    ...
    try {
      ...
      setProfileIsFollowed(
        Array.isArray(u.followers) ? u.followers.map(String).includes(String(user?._id)) : false,
      );
    } catch (err) {
  ...
  function renderMessages() {
    return messages.map((m, index) => {
      ...
      const unreadMark = unreadIndex >= 0 && index === unreadIndex ? true : false;

      const senderStatus = statuses[String(m.sender?._id || m.sender?.id)] || null;
      ...
      return (
        <Fragment key={m._id}>
          ...
          {unreadMark && (
            <div ref={unreadRef} className="my-2 flex justify-center text-xs text-white">
              <span className="px-3 py-1 rounded-md bg-orange-500">Unread messages</span>
            </div>
          )}
          ...
          >
            ...
            <div
              className={clsx('message', String(m.sender?._id) === String(user?._id) && 'msg-mine')}
              onClick={onMessageContainerClick}
            >
              <div className="flex items-center gap-2">
                {String(m.sender?._id) !== String(user?._id) && showAvatar && (
                  <strong>{m.sender?.username}</strong>
                )}
                <span
                  className="text-xs opacity-60"
                  title={dayjs(m.createdAt).format('dddd, DD MMM YYYY • HH:mm')}
                >
                  {dayjs(m.createdAt).format('HH:mm')}
                </span>

                {m.edited && <span className="text-xs opacity-50 ml-1">(edited)</span>}
              ...
              {m.fileUrl && (
                <img
                  ...
                  onClick={() =>
                    openMessageImage(m.fileUrl.startsWith('http') ? m.fileUrl : API + m.fileUrl)
                  }
                />
              )}
              ...
              {editingMessageId === m._id ? (
                <div className="flex gap-2 items-start mt-2">
                  ...
                  <button className="px-3 py-2 border rounded-md" onClick={cancelEdit}>
                    Cancel
                  </button>
                </div>
              ) : (
                <>
                  {m.text && (
                    <div
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setExpandedMessages((prev) => ({ ...prev, [m._id]: !prev[m._id] }));
                      }}
                      style={
                ...
                >
                  ...
                  {String(m.sender?._id) === String(user?._id) && (
                    <div className="ml-2 flex gap-2 text-xs">
                      ...
                      <button onClick={() => deleteMessageForEveryone(m._id)}>
                        Delete for everyone
                      </button>
                    </div>
                  )}
  ...
  const [messageImageViewer, setMessageImageViewer] = React.useState<{ index: number } | null>(
    null,
  );
  ...
  function closeMessageImage() {
    setMessageImageViewer(null);
  }
  function prevMessageImage() {
    if (!messageImageUrls.length) return;
    setMessageImageViewer((v) => ({
      index: ((v?.index || 0) + messageImageUrls.length - 1) % messageImageUrls.length,
    }));
  }
  function nextMessageImage() {
    if (!messageImageUrls.length) return;
    setMessageImageViewer((v) => ({ index: ((v?.index || 0) + 1) % messageImageUrls.length }));
  }
        ...
        {view === 'chat' && (
          <div className="h-full grid grid-rows-[auto_1fr_auto]">
                  ...
                  >
                    ...
                    >
                      <Zap
                        className={clsx(
                          'w-4 h-4',
                          enterToSend ? 'text-indigo-500' : 'text-gray-400',
                        )}
                      />
                      ...
                      onClick={async () => {
                        ...
                        try {
                          if (inDM && activeConversation) {
                            ...
                            await axios.delete(`${API}/api/messages/room/${room}/clear`, {
                              headers: { Authorization: `Bearer ${token}` },
                            });
                          }
                          setMessages([]);
                      ...
                      <>
                        ...
                        <button
                          onClick={async () => {
                            if (
                              !confirm(
                                'Delete this entire conversation permanently? This cannot be undone.',
                              )
                            )
                              return;
                            try {
            ...
            {showLongPressHintChat && (
              <div
                className="mx-4 mt-2 mb-0 rounded-lg px-3 py-2 text-xs bg-gradient-to-r from-indigo-500/10 to-emerald-500/10 border"
                style={{ borderColor: 'var(--border)' }}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-theme-secondary">
                    Tip: Long-press the header for chat options.
                  </span>
                  <button
                    className="text-[11px] px-2 py-1 rounded-md bg-white/60 dark:bg-slate-700/60 hover:opacity-80"
                    onClick={() => {
                      try {
                        localStorage.setItem('auralink-hint-chat-longpress', 'true');
                      } catch {}
                      setShowLongPressHintChat(false);
                    }}
                  >
                    Got it
                  </button>
                    ...
                    {Array.from({ length: 8 }).map((_, i) => (
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          />
                          <div
                            className="mt-2 h-3 w-2/5 rounded"
                            style={{ background: 'var(--muted)' }}
                          />
                        </div>
                      </div>
            ...
            >
              {replyTo && (
                <div
                  className="mb-2 p-2 rounded-md border text-xs flex items-center gap-2"
                  style={{ borderColor: 'var(--border)', background: 'var(--card)' }}
                >
                  <span className="opacity-80">Replying to</span>
                  <strong>{replyTo?.sender?.username || 'User'}</strong>
                  <span className="opacity-70 truncate max-w-[50%]">
        ...
        >
          ...
          >
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={clearAllImages} className="px-4 py-2 border rounded-md">
                Clear All
              </button>
              <button onClick={() => setImagePreviewOpen(false)} className="btn px-4 py-2">
                Done
              </button>
            </div>
          </div>
