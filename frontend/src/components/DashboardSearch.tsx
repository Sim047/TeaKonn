...
export default function DashboardSearch({
      ...
      try {
        ...
        const [usersRes, eventsRes, postsRes, servicesRes, marketRes] = await Promise.all([
          ...
          axios.get(`${API}/api/events`, {
            params: { search: debouncedQ, category: 'sports' },
            headers,
          }),
        ...
        const others: ResultItem[] = (postsRes.data?.posts || postsRes.data || [])
          .filter((p: any) =>
  ...
  return (
    <div>
      <div
        className="w-full flex items-center gap-2 px-4 py-3 rounded-xl border"
          ...
          <div className="space-y-1 max-h-80 overflow-y-auto">
            {results.map((r) => (
              <button
                ...
                className="w-full text-left rounded-lg themed-card hover:shadow-sm flex items-center gap-3 p-3"
              >
                {iconFor(r.type)}
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-heading truncate">{r.title}</div>
                  {r.subtitle && (
                    <div className="text-theme-secondary text-xs truncate">{r.subtitle}</div>
                  )}
                </div>
                <div
